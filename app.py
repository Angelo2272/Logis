...
st.markdown("""
    ...
    /* Botones elegantes: Azul pizarra oscuro */
    .stButton>button { 
        width: 100%; 
...
# Selector de Usuario (Fundamental para las estadísticas)
        ...
        try:
            ...
            if not tickets_encontrados:
                ...
                for ticket_path in tickets_encontrados:
                    # La IA analiza el archivo (PDF o Imagen)
                    datos = extraer_datos_foto(ticket_path)
                    
                    # Guardamos usando el usuario seleccionado actualmente
                    guardar_movimiento(
                        datos['fecha'], 
    ...
    with st.spinner("Analizando documento..."):
        try:
            ...
            with st.container():
                ...
                # Botón de guardar DENTRO del bloque de validación
...
try:
    ...
    if not df.empty:
        ...
        with col_f1:
            # Extraemos todos los años disponibles en el Excel
        ...
        # 2. FILTRADO DINÁMICO
        df_filtrado = df[
            (df['fecha_limpia'].dt.month == mes_sel_num) & 
            (df['fecha_limpia'].dt.year == anio_sel)
        ]
        
        total_filtro = df_filtrado['total'].sum()

        # 3. MÉTRICAS DEL PERIODO SELECCIONADO
        st.markdown(f"#### Resumen de {meses_nombres[mes_sel_num]} {anio_sel}")
        
        c1, c2, c3 = st.columns(3)
        with c1:
            st.metric("Gasto Total", f"{total_filtro:.2f} €")
        with c2:
            st.metric("Nº Tickets", len(df_filtrado))
        with c3:
            total_h = df['total'].sum()
            st.metric("Total Histórico", f"{total_h:.2f} €")

        # 4. GRÁFICAS DEL MES SELECCIONADO
        if total_filtro > 0:
            ...
            with st.expander(f"Ver tickets de {meses_nombres[mes_sel_num]}"):
                st.dataframe(
                    df_filtrado[['fecha', 'establecimiento', 'total', 'categoria', 'usuario']].sort_values(by='fecha', ascending=False),
